...
RSpec.describe SwitchOrgsController do
  ...
  describe "GET #switch_to" do
    context "when user is a member of the org" do
      let!(:member) { create(:member, org: org, user: user) }

      context "when project and participation are present" do
      ...
      context "when project is not present or user is not a participant" do
    ...
    context "when user is not a member of the org" do
